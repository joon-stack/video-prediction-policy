...
# 1) 옵티마이저 (LoRA 파라미터만 대상)
optim_ = torch.optim.AdamW(lora_params, lr=LR)
...
for epoch in range(EPOCHS):
    for batch in dataloader:               # ← 실제 데이터로더로 교체
        images, prompts = batch            # images: (B,3,H,W) , prompts: list[str]

        # ---------------------------------------------------------------
        # forward(): (예) ① VAE encode → ② noise 추가 → ③ UNet 예측
        # loss   = MSE(predicted_noise, true_noise)
        # ---------------------------------------------------------------
        # ... forward-pass / loss 계산 코드 작성 ...

        loss.backward()                    # backward
        optim_.step()                      # 파라미터 업데이트
        optim_.zero_grad()                 # gradient 초기화
