...
def main(
    ...
    *_, config = inspect.getargvalues(inspect.currentframe())
    ...
    if 'clip' in train_args.clip_model_path:
        ...
        text_encoder = CLIPTextModelWithProjection.from_pretrained(train_args.clip_model_path, use_safetensors=True)
        ...
        if train_args.use_img_cond:
            ...
            image_encoder = CLIPVisionModelWithProjection.from_pretrained(train_args.clip_model_path, use_safetensors=True)
        ...
        tokenizer = T5Tokenizer.from_pretrained(train_args.clip_model_path, use_safetensors=True)
        # model = T5ForConditionalGeneration.from_pretrained("/cephfs/shared/llm/t5-v1_1-large").to("cuda")
        text_encoder = T5EncoderModel.from_pretrained(train_args.clip_model_path, use_safetensors=True)
    ...
    def collect_full_path_targets(unet, branch: str):
        """
        LoRA를 적용할 Linear 모듈의 이름을 수집합니다.
        branch: "spatial", "temporal", or "both"
        """
        assert branch in ("spatial", "temporal", "both")
        leaves = ("to_q", "to_k", "to_v", "to_out.0")
        targets = []
        ...
        for name, module in unet.named_modules():
            ...
            # 2) down_blocks, mid_block.attentions, up_blocks 접두사가 있는지
            in_block = any(seg in name for seg in (
                "down_blocks.",        # down_blocks.0.attentions...
                "mid_block.attentions.",# mid_block.attentions...
                "up_blocks."           # up_blocks.1.attentions...
            ))

            # 3) spatial vs temporal vs both
            is_spatial  = in_block and "transformer_blocks" in name  and "temporal_transformer_blocks" not in name
            is_temporal = in_block and "temporal_transformer_blocks" in name

            if branch == "spatial"  and not is_spatial:   continue
            if branch == "temporal" and not is_temporal:  continue
            if branch == "both"     and not (is_spatial or is_temporal):  continue

            # 4) to_q, to_k, to_v, to_out.0 로 끝나는지
            if any(name.endswith(leaf) for leaf in leaves):
                targets.append(name)
    ...
    target_modules = collect_full_path_targets(unet, branch="temporal")
    ...
    def print_learnable_parameters(model):
        """
        Prints each trainable parameter name with its size,
        then a summary: trainable vs total params and percentage.
        """
        total_params = 0
        trainable_params = 0
    ...
    print_learnable_parameters(unet)
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            if accelerator.sync_gradients:
                ...
                if global_step % checkpointing_steps == 0 and accelerator.is_main_process:
                    ...
                    ckpt_path = os.path.join(output_dir, f"checkpoint-{global_step}", "training_state.pt")
                    torch.save(ckpt, ckpt_path)
                    logger.info(f"Saved training state at {ckpt_path}")
